...
const animColor = "#573217";
const monthColor = "#B19162";
...
const TimelineSection = ({ isDesktop }: IDesktop) => {
  ...
  const getDotString = (x: number, y: number, timelineNode: LinkedCheckpointNode) => {
    const { month } = timelineNode;
    let monthSvg = '';
    if (month !== undefined) {
        monthSvg = `<rect width="40" height="25" fill="#000" x=${x - 20} y=${y + 28}></rect>
        <text x=${x - 10} y=${y + 43} font-size="12" fill=${monthColor}>${month}</text>`;
    }
    return `${monthSvg}
  ...
  const addText = (
    ...
    const { title, subtitle, month, size, image } = timelineNode;
    ...
    const titleSizeClass = size === ItemSize.LARGE ? "text-6xl" : "text-2xl text-gradient";
